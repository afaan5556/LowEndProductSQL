...
AS (SELECT
  ...
  b.description AS at_line_item,
  initial_cost / project_total.project_usf AS at_budget_line_per_usf, -- get per usf # so when we join to dpr we mult by square feet on that floor/ area 
...
LEFT JOIN project_total
  ON p.uuid = project_total.project_uuid
WHERE at_revision.rn = 1 -- Only keep latest revision
AND project_total.project_uuid IS NOT NULL  -- only if there is a project with more than 0 square feet
),
...
AS (SELECT
  p.uuid AS project_uuid,
  purchase_item AS jcr_line_item,
  cpi AS jcr_cpi,
  -- Actuals and projected jcr columns added
  total_paid_amount_paid_expense_report_credit_card_intercompany_ / project_total.project_usf AS jcr_actuals_per_usf,
  total_projected_spend_po_supplier_contract_ / project_total.project_usf AS jcr_projected_per_usf,
  invoiced_amount / project_total.project_usf AS jcr_invoiced_amount_per_usf,
  expense_report / project_total.project_usf AS jcr_expense_report_per_usf,
  current_budget / project_total.project_usf AS jcr_budget_line_per_usf
FROM workday_development.mv_workday_job_cost_report jcr
LEFT JOIN stargate_bi_stargate.bi_project p
  ON p.id = jcr.project_id
LEFT JOIN project_total
  ON p.uuid = project_total.project_uuid
WHERE project_total.project_uuid IS NOT NULL
),
...
CASE WHEN SUM(rd.final_cost) != SUM(rd.initial_cost) THEN SUM(rd.initial_cost::NUMERIC(30,20))/SUM(rd.final_cost) END AS rate
...
AS (SELECT 
b.project_uuid,
LOWER(sg_revision_detail.cpi_categories) AS sg_cpi,
(CASE WHEN sg_revision_detail.rate IS NULL AND r.rate IS NULL THEN 1 
WHEN sg_revision_detail.rate IS NULL THEN r.rate ELSE sg_revision_detail.rate END ) AS sg_exchange_rate,
date_trunc('day',sg_revision.created_date ) AS sg_budget_date,
/*Apply rate to budget*/
sg_revision_detail.budget * (CASE WHEN sg_revision_detail.rate IS NULL AND r.rate IS NULL THEN 1 
WHEN sg_revision_detail.rate IS NULL THEN r.rate ELSE sg_revision_detail.rate END ) / project_total.project_usf AS sg_cpi_budget_per_usf,
/*Apply rate to ti*/
b.proforma_ti_allowance * (CASE WHEN sg_revision_detail.rate IS NULL AND r.rate IS NULL THEN 1 
WHEN sg_revision_detail.rate IS NULL THEN r.rate ELSE sg_revision_detail.rate END ) / project_total.project_usf as sg_ti_per_usf,
...
LEFT JOIN project_total
  ON b.project_uuid = project_total.project_uuid

WHERE sg_revision.rn = 1 AND sg_cpi_budget_per_usf IS NOT NULL AND sg_cpi_budget_per_usf != 0
...
AS (SELECT
  ...
  COALESCE(airtable.at_line_item, workday.jcr_line_item) AS line_item, -- and here 
  airtable.at_sign_date,
  airtable.at_budget_line_per_usf,
  airtable.at_currency,
  stargate.sg_budget_date,
  stargate.sg_cpi_budget_per_usf,
  stargate.sg_ti_per_usf,
  stargate.sg_currency,
  workday.jcr_invoiced_amount_per_usf,
  workday.jcr_expense_report_per_usf,
  workday.jcr_budget_line_per_usf,
  workday.jcr_actuals_per_usf,
  workday.jcr_projected_per_usf

FROM airtable
FULL JOIN workday
  ON airtable.project_uuid = workday.project_uuid
  AND LOWER(airtable.at_cpi) = LOWER(workday.jcr_cpi)
  AND airtable.at_line_item = workday.jcr_line_item
 FULL JOIN stargate
  ON airtable.project_uuid = stargate.project_uuid
  AND LOWER(airtable.at_cpi) = LOWER(stargate.sg_cpi)
)
...
SELECT
  ...
  SUM(sg_ti_per_usf) AS sg_ti_per_usf_total,
